...
public:
    ...
    // a helper function for display array
    void array_display_left(Node *ptr)
    {
        if (ptr != NULL)
        {
            cout << " " << ptr->data << " ";
            array_display_left(ptr->left);
            array_display_left(ptr->right);
        }
    }
    // a helper function for display array
    void array_display_right(Node *ptr)
    {

        if (ptr != NULL)
        {
            cout << " " << ptr->data << " ";
            array_display_right(ptr->right);
            array_display_right(ptr->left);
        }
    }
    // A function to diplay in order
    void display_array(Node *ptr)
    {
        // left -- root-- right
        // Node* temp = root;
        if (ptr == NULL)
            return;
        cout << "\n";
        array_display_left(ptr);
        cout << "\n";
        array_display_right(ptr);
        cout << "\n";
        display_array(ptr->left);
        array_display_left(ptr->left);
        cout << "\n";
        array_display_right(ptr->right);
    }
...
int main()
{
    ...
    cout << "\n Counting the Possible Array's";
    tree.display_array(tree.root);
    return 0;
}